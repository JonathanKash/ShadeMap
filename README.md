...
**Top 10 near campus and top 10 far from campus** ([`outputs/priority_near_far_campus.md`](outputs/priority_near_far_campus.md), and two chips in the app view). A single citywide ranking is dominated by University of Florida campus stops, so the rest of Gainesville disappears from the top. Instead the app shows two top 10 lists, ranked with the same score: stops within 1 mile of the UF campus (campus included) and stops farther than 1 mile. Distance is measured from the edge of the main campus, using its OpenStreetMap boundary, not from a center point (the separate 20-acre East Campus site is not used, because it sits 2.7 miles away and would pull unrelated northeast Gainesville stops into "near campus"). 369 stops are near campus and 602 are far from it, and the far stops carry 36 percent of weekday bus visits. Choosing either chip draws the campus outline and the one-mile line on the map. The far-from-campus top 10 (Oaks Mall and apartment stops on routes 75, 52 and 15 in northwest Gainesville) ranks 51st to 77th in the citywide list, so a single top 20 would never have shown them. Near campus includes student apartments and much of downtown and Midtown, and shelter status is OpenStreetMap-based and partial.
...
**Result counts:** 971 stops, 850 ranked, 69 excluded because OpenStreetMap already
marks them sheltered, 39 excluded for no satellite data, 13 excluded for no weekday
service. The ranking is a queue for new shade, so a stop that has a shelter is normally
not in it, with one exception: a sheltered stop in the busiest tenth of the network (95
or more bus visits a day, 37 stops such as Reitz Union and the transfer stations) stays
eligible, because one shelter cannot cover the crowds that level of service implies. We
cannot measure shelter capacity, so service volume is the overflow proxy. Stops with
unknown shelter status stay eligible: OSM coverage is thinnest in under-mapped
